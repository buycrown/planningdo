...
function render(){
  ...
  // 저장된 AI PLAN 결과가 있으면 유지(재실행 전까지 기억) · 없으면 안내문구
  const recoBox = document.getElementById('recoBox');
  const recoEmpty = document.getElementById('recoEmpty');
  if (recoBox && recoEmpty) {
    if (window.AIPlan && (AIPlan._last || (AIPlan._restore && AIPlan._restore())) && AIPlan._renderSummary) {
...
function card(e,score,hits){
  ...
  // 속성 칩을 축(구좌/혜택/상품/브랜드/…)별로 분리해 라벨과 함께 표시
  const kwHtml=AXES.map(a=>{
    if(a.key==='card') return '';   // '🃏 카드 구성' 전용 블록과 중복 → 칩 그룹에서는 제외
    const vals=e.kw[a.key]||[];
    if(!vals.length) return '';
    const chips=vals.map(v=>`<span class="kw ${a.key}${hits.has(v)?' match':''}" style="background:${a.color}22;color:${a.color};border:1px solid ${a.color}40">${escHtml(v)}</span>`).join('');
    return `<div class="kwgroup"><span class="kwax" style="color:${a.color}">${a.label}</span><span class="kwchips">${chips}</span></div>`;
  }).join('');
  ...
  <div class="card">
    ...
    <div class="body">
      ...
      ${((e.products&&e.products.length)||(e.tabs&&e.tabs.length)||(e.card_types&&e.card_types.length)) ? `
      <div class="planParts" style="margin-top:7px;border-top:1px dashed var(--line2);padding-top:7px;font-size:11px;line-height:1.65">
        ${e.products&&e.products.length ? `<div style="margin-bottom:2px"><b style="color:var(--sub)">🛍️ 대상 상품</b> ${e.products.slice(0,3).map(escHtml).join(' · ')}${e.prodCnt>3?` <span style="color:var(--muted)">외 ${e.prodCnt-3}개</span>`:''}</div>` : ''}
        ${e.tabs&&e.tabs.length ? `<div style="margin-bottom:2px"><b style="color:var(--sub)">📑 기획전 내 탭</b> ${e.tabs.slice(0,5).map(t=>`<span class="kw">${escHtml(t)}</span>`).join('')}${e.tabs.length>5?` <span style="color:var(--muted)">+${e.tabs.length-5}</span>`:''}</div>` : ''}
        ${e.card_types&&e.card_types.length ? `<div><b style="color:var(--sub)">🃏 카드 구성</b> ${e.card_types.map(c=>`<span class="kw" style="background:rgba(253,186,116,.18);border-color:#fdba74">${escHtml(c)}</span>`).join('')}</div>` : ''}
      </div>` : ''}
      ${reviewHtml}
      <div class="cardActions">
        <button class="btn sm" onclick="showRelated('${e.id}')">🧬 유사 기획전</button>
        <button class="btn sm sec" onclick="PlanDetail.open('${e.id}')">🔍 상세·AI INSIGHT</button>
...
function renderCompare(){
  ...
  const rows=[
    ...
    ["통합전 여부", e=>e.multi_brand_yn==='Y'||e.multi_brand_yn===true?`통합전(${e.brand_count})`:"단일브랜드", true],
    ["메인카테고리", e=>ENUM_LABELS.main_category[e.main_category]||e.main_category, true],
    ["쿠폰유형", e=>ENUM_LABELS.coupon_type[e.coupon_type]||e.coupon_type, true],
    ["접근루트", e=>ENUM_LABELS.access_route[e.access_route]||e.access_route, true],
