...
입력: ../PLAN_DATABASE/* , ../prep/prodinfo_slim.jsonl, ../prep/brand_slim.jsonl
7축: placement·theme·benefit·product·brand·card(카드구성)·metric"""
...
# 날짜 폴더(YYYYMMDD)가 있으면 최신 폴더를 데이터 기준으로 자동 선택, 없으면 루트 사용
...
def load_json(name):
    ...
    try:
        ...
        if os.path.abspath(alt) != os.path.abspath(fp) and os.path.exists(alt):
            print("    [경고] " + name + " 파싱 실패(" + type(e).__name__ + ") → 상위 PLAN_DATABASE 복사본 사용")
...
print("[2] LST_PLAN_PROD_PLANSQ (PLAN_SQ↔PROD_CD 직접, 라인파싱)")
...
print("[6] 카드구성(TB_LFM12_EXHB_CARD_LIST) / 탭(LST_PLAN_DTL) / category / coupon / display / revenue")
CARD_LABEL = {
...
plan_cards = defaultdict(list)   # plan -> [(sort_seq, label, raw)]
...
# 수동 쿠폰 오버라이드: 페이지 게시(퍼블리싱) 쿠폰이 있으나 DB 미매핑된 기획전 보정
COUPON_OVERRIDE = {}
_ov_fp = os.path.join(OUT, "coupon_override.json")
if os.path.exists(_ov_fp):
    try:
        with open(_ov_fp, encoding="utf-8") as f:
            COUPON_OVERRIDE = (json.load(f) or {}).get("overrides", {})
        if COUPON_OVERRIDE:
            print("    [쿠폰 오버라이드] " + ", ".join(sorted(COUPON_OVERRIDE.keys())))
    except Exception as e:
        print("    [경고] coupon_override.json 로드 실패: " + type(e).__name__)
...
for psq in sorted(KEYS, key=lambda k: int(k)):
    ...
    _ov = COUPON_OVERRIDE.get(psq)
    coupon_type = "쿠폰있음" if (cps or (_ov and _ov.get("coupon_type") == "쿠폰있음")) else "쿠폰없음"
    ...
    if _ov and _ov.get("coupon_kw") and _ov["coupon_kw"] not in coupon_kw:
        coupon_kw.append(_ov["coupon_kw"])
    ...
    if not conv:   # '상품UV CR' 결측(전 행 null) → 주문건수/상품UV로 전환율 산출
...
print("[8b] 기획전별 상품 상세 파일 (products/{plan}.json)")
...
CONFIG={"SYN":oc.get("SYN",{}),"AXES":AXES,
 ...
 "models":[
   {"id":"gemini-3.1-pro-preview","name":"Gemini 3.1 Pro (유료·결제연결)"},
   {"id":"gemini-3-pro-preview","name":"Gemini 3 Pro (유료)"},
   {"id":"gemini-2.5-flash","name":"Gemini 2.5 Flash (무료·자동, 기본)"},
   {"id":"gemini-2.5-pro","name":"Gemini 2.5 Pro (API)"},
   {"id":"claude-opus-4-7","name":"Claude Opus 4.7 (유료·최고성능)"},
 ...
 "gemini_api_key":oc.get("gemini_api_key",""),"claude_api_key":oc.get("claude_api_key",""),"openrouter_api_key":oc.get("openrouter_api_key",""),
