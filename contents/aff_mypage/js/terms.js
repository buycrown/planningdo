 ...
 * 문안은 어필리에이트 가입 신청 페이지의 약관 모달 원문과 동일하게 유지.
 */
(function () {
  'use strict';
  ...
  function docHTML(key) {
    ...
    return '' +
      '<div class="agree-bar">' +
        '<span>' + d.meta.version + ' · 가입 시 동의 완료</span>' +
        '<b>' + d.meta.agreedAt + '</b>' +
      '</div>' +
      '<div class="terms-doc">' +
        d.sections.map(function (s) { return '<h3>' + s[0] + '</h3><p>' + s[1] + '</p>'; }).join('') +
        '<p class="poc-note">' + POC_NOTE + '</p>' +
      '</div>';
  }

  var body =
    '<div class="tabs" id="tabs">' +
      '<button type="button" class="is-on" data-doc="terms">서비스 이용약관</button>' +
      '<button type="button" data-doc="policy">서비스 운영정책</button>' +
    '</div>' +
    '<div id="docArea">' + docHTML('terms') + '</div>';

  LF.mountScreen({ title: '이용약관', back: 'affiliate.html' }, body);
  LF.mountDemoTools();

  LF.$('#tabs').addEventListener('click', function (e) {
    var btn = e.target.closest('button');
    if (!btn) return;
    LF.$$('#tabs button').forEach(function (b) { b.classList.toggle('is-on', b === btn); });
    LF.$('#docArea').innerHTML = docHTML(btn.dataset.doc);
    LF.$('#scroll').scrollTop = 0;
  });
})();
