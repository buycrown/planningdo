...
(function (global) {
  ...
  /* ------------------------------------------------------------------
     0. 공용 인증 모듈 설정 (경로는 03_활동내역 폴더 기준 상대경로)
     ------------------------------------------------------------------ */
  var LOGIN_URL = '../04_로그인/login.html';

  if (global.LFAuth && typeof global.LFAuth.config === 'function') {
    global.LFAuth.config({
      loginUrl: LOGIN_URL,
      homeUrl: 'affiliate.html',
    ...
    if (!opt.logout) {
      ...
      if (!opt.back) {
        back.addEventListener('click', function (e) {
          ...
          else global.location.href = 'affiliate.html';
        });
      }
