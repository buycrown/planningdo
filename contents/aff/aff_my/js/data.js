...
(function (global) {
  ...
  var Store = {
    ...
    updateContact: function (patch) {
      ...
      return global.LFAuth.api('userUpdateContact', body).then(function (res) {
        res = res || {};
        /*
         * [v2.2] 이메일을 바꾸면 회원ID(profile.id)가 바뀐다.
         *  · memberIdChanged:true  → 세션 프로필의 id 를 새 값으로 교체하고
         *                            LFAuth.clearAffiliateCache() 로 캐시를 지운 뒤 다시 받는다.
         *                            (갱신하지 않으면 캐시의 memberId 가 옛 값이라
         *                             세션 불일치 폐기 로직에 걸려 화면이 튕긴다)
         *  · memberIdChanged:false → 표기(대소문자)만 바뀐 것이므로 기존 동작 유지.
         */
        var memberIdChanged = res.memberIdChanged === true;
        var newMemberId = res.memberId === undefined || res.memberId === null ? '' : String(res.memberId);

        if (_data && _data.profile) {
          /* 화면 표시는 원본표기(profile.email)를 쓴다 — 사용자가 방금 입력한 값 그대로 */
          ...
          if (newMemberId) _data.profile.id = newMemberId;
        }

        /* 세션이 다시 묶이지 않았다면(sessionRebound:false) 토큰은 이미 무효다 → 재로그인 */
        if (memberIdChanged && res.sessionRebound === false) {
          var a0 = auth();
          if (a0 && typeof a0.clearSession === 'function') { try { a0.clearSession(); } catch (e) {} }
          var e0 = new Error('회원ID가 변경되어 세션이 종료되었습니다. 다시 로그인해 주세요.');
          e0.code = 'SESSION_REBIND_REQUIRED';
          return Promise.reject(e0);
        }

        if (memberIdChanged && newMemberId) {
          var a = auth();
          if (a && typeof a.updateSessionProfile === 'function') {
            try { a.updateSessionProfile({ id: newMemberId, email: body.email || (_data && _data.profile ? _data.profile.email : '') }); } catch (e) {}
          }
          /* 회원ID 가 바뀌었으므로 옛 캐시(LFAuth.clearAffiliateCache)를 지우고 다시 받는다. */
          var prev = _data;
          cacheDrop();
          _raw = null;
          _refreshing = null;
          return Store.load(true).then(function () {
            return Store.profile();
          }, function () {
            /* 재조회 실패 : 저장 자체는 성공했으므로 화면을 비우지 않는다 */
            if (!_data) { _data = prev; _promise = null; }
            return Store.profile();
          });
        }
